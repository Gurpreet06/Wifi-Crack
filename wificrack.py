...
# Script Banner
def script_banner():
    owner_name = 'Original Author: Gurpreet ~ Singh (Gurpreet06)'
    contributor = 'Updated By: Arkhalis'
    owner = f"""
    
            __      _____ ___ ___   /\/|   ___ ___    _   ___ _  __  \t┌───────────────────────────────────────────────────┐
     \ \    / /_ _| __|_ _| |/\/   / __| _ \  /_\ / __| |/ /         \t│  {Fore.BLUE + owner_name}   │        
      \ \/\/ / | || _| | |        | (__|   / / _ \ (__| ' <          \t│                                                   │     
       \_/\_/ |___|_| |___|        \___|_|_\/_/ \_\___|_|\_\         \t│  {contributor}                             │
                                                                     \t└───────────────────────────────────────────────────┘
    """
...
# Check for hcxpcapngtool
    check_hcxpcapngtool = subprocess.run(["which", "hcxpcapngtool"], capture_output=True, text=True)
    if "hcxpcapngtool" not in check_hcxpcapngtool.stdout:
        get_colours(f"\n[!] Install the '[hcxpcapngtool]' package from the following github repository: ", 'red')
        print(f"\n{Fore.BLUE + '┃'}  {Fore.YELLOW + ' git clone https://github.com/ZerBea/hcxtools'}")
...
def attack_func(network_interface, attack_mode):
    ...
    # PKMID Attack Mode
        ...
        get_colours("", "yellow")
        subprocess.run(["clear"])
        get_pkmid_hashes = subprocess.run([f"sudo hcxpcapngtool -z wifiOutput/PKMID/myHashes_PKMID wifiOutput/PKMID/Capture_PKMID"],
